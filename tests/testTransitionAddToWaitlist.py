...
class TestTransitionAddToWaitlist(UWOshOIETestCase):
    ...
    def test_program_manager_should_be_allowed(self):
    ...
    def test_should_throw_exception_when_other_roles_attempt_to_add_to_waitlist(self):
        app = self.createApplication()
        
        for user in self._oie_users.keys():
            if user != 'program_manager':
                self.login(user)
                self.assertRaises(WorkflowException, self.portal_workflow.doActionFor, app, 'addToWaitlist')
                self.logout()
...
def test_suite():
    ...
    suite = TestSuite()
    suite.addTest(makeSuite(TestTransitionAddToWaitlist))
